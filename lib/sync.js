...
class Sync {
    ...
    async getData(user, course_id) {
        ...
        const getWithApi = async (item) => {
            ...
            while (true) {
                try {
                    let page_result = await oauth.get(['api', 'v1', 'courses', course_id, item + '?per_page=100&page=' + page], user.access_token);
    ...
    async syncCourse(course_id) {
        ...
        while (true) {
            // Get a random user available for the course
            user_id = null;
            user = null;
            let error;
            try {
                await database.select('user_id').from('user_course').where('course_id', course_id).then(rows => {
                    if (rows.length > 0) user_id = rows[Math.floor(Math.random() * rows.length)].user_id;
                }).catch(e => {
                    error = e;
                });
                if (error)throw error;
                if (user_id) {
                    user = await _user.getUser(user_id);
                }
            } catch (error) {
            ...
            if (!user) {
                if (user_id) {
                    log('warn', `User (id: ${user_id}) not found, remove this user`);
                    try {
                        await database('user_course').where('user_id', user_id).delete().then().catch(e => {
                            error = e;
                        });
                        if (error)throw error;
                    } catch (error) {
            ...
            log('info', `Got a random User(${user.id}, ${user.name}) from the list`);
            ...
            return async function (callback) {
                ...
                if (file.created || file.modified) {
                    ...
                    try {
                        ...
                        let err = fs.writeFileSync(path.resolve(utils.local_file_dir, file.filename), buffer);
                        if (err) throw err;

                        if (config.has('qiniu.enable') && config.get('qiniu.enable')) {
                            const hash = await qiniu.upload(utils.local_file_dir, file.filename);
                        ...
                        if (config.has('sync.preserve_local_file') && !config.get('sync.preserve_local_file')) {
                            await fs.unlink(path.resolve(utils.local_file_dir, file.filename), async err => {
                                if (err) throw err;
                            });
                            log('info', `File(${file.id}) Removed from disk`);
                        }
        ...
        await database.raw(raw).then(data => {
            if (data.length > 0) {
                ...
                num_created = processed.num_created;
                num_modified = processed.num_modified;
        ...
        await database('course').where({id: course_id}).update({
            sync_time: database.fn.now(),
            sync_user_id: user.id
        }).then(data => {
            log('info', `Sync Time and User(${user.id}) updated`);
        }).catch(e => {
            log('warn', e);
        });
