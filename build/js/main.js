...
var filesize = __webpack_require__(5);
var fileicon = __webpack_require__(4);
var sillydatetime = __webpack_require__(9);
...
var FileView = React.createClass({
    ...
    onSyncClick: function onSyncClick() {
        ...
        var timestamp = this.timestamp = new Date();
        var times = 10;
        var interval = 2000;
        var i = 0;
        var loop = setInterval(function () {
            if (timestamp !== _this4.timestamp) {
                // Course changed
            ...
            if (++i === times) {
                window.clearInterval(loop);
                _this4.setState({ sync: false, sync_timeout: true });
                return;
            }
            $.ajax({
                url: '/data/sync',
                data: {
                    id: _this4.state.course.id,
                    time: Date.parse(_this4.state.course.last_sync_time)
                },
                type: 'GET',
                dataType: 'json',
                success: function success(data) {
                    ...
                    if (data.state == 0 && !_this4.state.sync) {} else if (data.state == 1) {
                        _this4.refreshTree(_this4.state.course.id, true);
                        window.clearInterval(loop);
                    }
                }
            });
    ...
    render: function render() {
        ...
        var first = [React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    'a',
                    { className: 'btn btn-primary btn-lg', href: '/auth', role: 'button' },
                    'Grant Privilege'
                )
            )
        )];
        ...
        var no_error = [React.createElement(
            ...
            React.createElement(
                ...
                '\xA0',
                this.state.sync ? "Syncing" : this.state.sync_timeout ? "Sync timeout" : this.state.sync_success ? "Synced" : "Sync now"
            )
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                this.state.error || this.state.first ? "" : React.createElement(FileTree, { callbackChangeFolder: this.callbackChangeFolder,
    ...
    refreshTree: function refreshTree(course_id) {
        ...
        var sync_success = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        ...
        $.ajax({
            ...
            success: function success(data) {
                if (data.course) {
                    ...
                    if (data.sync) _this5.onSyncClick();
                } else {
                    callbackError(data);
                }
                console.log(data);
...
(function (global, factory) {
  ...
  ( false ? 'undefined' : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(exports) :  true ? !(__WEBPACK_AMD_DEFINE_ARRAY__ = [exports], __WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
				__WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ?
				(__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__)) : global.async ? factory(global.neo_async = global.neo_async || {}) : factory(global.async = global.async || {});
  ...
  function asyncify(fn) {
    return function () /* args..., callback */{
      ...
      if (result && (typeof result === 'undefined' ? 'undefined' : _typeof(result)) === obj && _typeof(result.then) === func) {
